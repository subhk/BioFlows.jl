...
@testset "BioFlows example constructors" begin
    ...
    tmp = mktempdir()
    writer = CenterFieldWriter(joinpath(tmp, "snapshots.jld2"); interval=0.05f0)
    for _ in 1:3
        sim_step!(circle; remeasure=false)
    ...
    osc = oscillating_cylinder_sim(; n=3*2^4, m=2^5, ν=0.067f0, St=0.1f0, amplitude=0.15f0)
    @test isa(osc, BioFlows.Simulation)
    sim_step!(osc; remeasure=true)
    coeff = total_force(osc) ./ (0.5f0 * osc.L * osc.U^2)
    @test length(coeff) == 2

    include(joinpath(@__DIR__, "..", "examples", "torus_3d.jl"))
    donut = donut_sim(; n=2^4, ν=0.028f0, major_ratio=0.3f0, minor_ratio=0.08f0)  # Reduced from 2^5 for CI
    ...
    sphere = sphere_sim(; n=2^4, m=2^4, ℓ=2^4, ν=0.053f0)  # Reduced from 2^5 for CI
