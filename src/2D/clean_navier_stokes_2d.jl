...
function solve_navier_stokes_clean_2d!(state_new::SolutionState, state_old::SolutionState,
    ...
    advection_u = u_cc .* dudx + v_cc .* dudy
    advection_v = u_cc .* dvdx + v_cc .* dvdy
    ...
    dφdy_cc = interpolate_v_to_cell_center(dφdy, grid)
    
    # Final velocity
    u_new = u_star - dt * dφdx_cc
    v_new = v_star - dt * dφdy_cc
...
function demonstrate_clean_vs_traditional()
    ...
    println("dpdx, dpdy = grad(p, grid)")
    
    dpdx_clean, dpdy_clean = grad(state.p, grid)
    println("Result: ∇p computed at $(size(dpdx_clean)) and $(size(dpdy_clean))")
